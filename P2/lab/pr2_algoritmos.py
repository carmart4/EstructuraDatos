...
def OrdSeleccion (v:list)->int:
    ...
    for i in range(n-1): #para todas las posiciones
        ...
        for j in range(i+1,n):
            pasos += 1
            if v[j].GetFecha_Sig() < v[pos_min].GetFecha_Sig():                         # Fila modificada para comparar fecha. 
...
def Particion (v:list, izq:int, der:int)->int: 
    ...
    """
    pasos = 0 #Contador de pasos    
    piv = v[ (izq + der)//2 ].GetFecha_Sig()                                          # Fila modificada
    i = izq
    j = der
	#Contabilizar todos los pasos dentro de este bucle
    #incluida la condicion i<=j cuando es True
    while i <= j:
        while v[i].GetFecha_Sig() < piv: #parar si mal ubicado a la izquierda         # Fila modificada
            pasos += 1
            i += 1
        while v[j].GetFecha_Sig() > piv: #parar si mal ubicado a la derecha           # Fila modificada
            pasos += 1
            j -= 1
